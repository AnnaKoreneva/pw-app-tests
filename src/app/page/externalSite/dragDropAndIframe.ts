...
export class DragDropAndIframe {
    ...
    protected iFrame = (): Locator => this.page.locator("[rel-title = 'Photo Manager'] iframe");
    ...
    async dragAndDropMouse() {
        await this.iFrame().contentFrame().locator('li', { hasText: 'High Tatras 4' }).hover();
    ...
    async checkIsImagesInTrash() {
        await expect(this.iFrame().contentFrame().locator('[id = "trash"] ul li h5')).toHaveText([
            'High Tatras 2',
            'High Tatras 4',
        ]);
    }
