...
export class UsingTheGrid extends FormLayoutBase {
    ...
    async signIn(formData: iForms) {
        await AllureHelper.step(
            'Fill the email and password and click the Sign In button',
            async () => {
    ...
    async checkTheSignInFormData(formData: iForms) {
        await AllureHelper.step(
            'Check that email input and radiobutton selection ace correct',
            async () => {
                await expect(this.formCard().getByPlaceholder('Email')).toHaveValue(formData.email);
                await expect(
